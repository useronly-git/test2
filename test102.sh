...
if cmd_exists ss; then
    ...
    write_log "Всего активных TCP-соединений (ESTABLISHED): $TOTAL_TCP"
    if [ "$OS_TYPE" = "Darwin" ]; then
        # macOS netstat: -v показывает PID. Резолвим PID → имя процесса
        netstat -anv -p tcp 2>/dev/null | grep 'ESTABLISHED' | awk '{print $9}' | sort | uniq -c | sort -rn | head -10 | while read -r cnt pid; do
            # ps -p PID -o comm= возвращает имя команды (без пути)
            PNAME="$(ps -p "$pid" -o comm= 2>/dev/null || echo '')"
            if [ -z "$PNAME" ] || [ "$PNAME" = "-" ]; then
                # Пробуем полный путь и берём basename
                PNAME="$(ps -p "$pid" -o command= 2>/dev/null | awk '{print $1}' | xargs basename 2>/dev/null || echo "PID:$pid")"
            fi
            write_log "Процесс: $(printf '%-25s' "${PNAME:-PID:$pid}") | Соединений: $cnt"
        done
    else
