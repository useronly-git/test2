...
# Функция парсинга одного файла extensions.json
# Аргумент: $1 = путь к extensions.json, $2 = метка источника (для лога)
_parse_firefox_extensions() {
    local addons_file="$1"
    local source_label="$2"
    [ -f "$addons_file" ] || return
    FF_FOUND=1
    local profile_dir
    profile_dir="$(dirname "$addons_file")"
    local profile_name
    profile_name="$(basename "$profile_dir")"
    write_log "Профиль: $profile_name (${source_label})"
...
# Функция сканирования корневой директории Firefox
# Ищет: 1) profiles.ini → путь к профилю по умолчанию
#       2) прямой поиск extensions.json во всех поддиректориях (фоллбэк)
# Аргумент: $1 = путь к директории профилей (где лежит profiles.ini)
#           $2 = метка источника для лога (native/flatpak/snap)
scan_firefox_profiles() {
    local profiles_dir="$1"
    local source_label="$2"
    [ -d "$profiles_dir" ] || return
    write_log "  Проверяем Firefox: $profiles_dir [${source_label}]"

    local found_here=0

    # Шаг 1: Парсим profiles.ini для точного определения путей профилей
    local profiles_ini="$profiles_dir/profiles.ini"
    if [ -f "$profiles_ini" ]; then
        # Извлекаем все строки Path= (относительные пути к профилям)
        # IsRelative=1 означает относительный путь, IsRelative=0 — абсолютный
        local rel_path
        while IFS= read -r rel_path; do
            [ -z "$rel_path" ] && continue
            local full_profile_dir
            # Если путь абсолютный — используем как есть, иначе склеиваем
            case "$rel_path" in
                /*) full_profile_dir="$rel_path" ;;
                *)  full_profile_dir="$profiles_dir/$rel_path" ;;
            esac
            if [ -f "$full_profile_dir/extensions.json" ]; then
                _parse_firefox_extensions "$full_profile_dir/extensions.json" "$source_label"
                found_here=1
            elif [ -d "$full_profile_dir" ]; then
                write_log "    Профиль $(basename "$full_profile_dir"): extensions.json отсутствует (нет установленных расширений)"
                FF_PROFILE_FOUND=1
            fi
        done < <(awk -F= '/^Path=/ {print $2}' "$profiles_ini" 2>/dev/null)
    fi

    # Шаг 2: Фоллбэк — ищем extensions.json во всех поддиректориях через find
    # (на случай если profiles.ini нет или он повреждён)
    if [ "$found_here" -eq 0 ]; then
        local ext_file
        while IFS= read -r ext_file; do
            [ -n "$ext_file" ] && _parse_firefox_extensions "$ext_file" "$source_label"
        done < <(find "$profiles_dir" -maxdepth 3 -name 'extensions.json' -type f 2>/dev/null)
    fi
}
...
if [ "$OS_TYPE" = "Darwin" ]; then
    scan_firefox_profiles "$HOME_DIR/Library/Application Support/Firefox" "macOS native"
else
    # Стандартный путь (нативная установка из apt/pacman/dnf)
    scan_firefox_profiles "$HOME_DIR/.mozilla/firefox" "native"
    # Flatpak (SteamOS, Fedora Silverblue, Ubuntu 24+ и др.)
    scan_firefox_profiles "$HOME_DIR/.var/app/org.mozilla.firefox/.mozilla/firefox" "flatpak"
    # Snap (Ubuntu 22.04+)
    scan_firefox_profiles "$HOME_DIR/snap/firefox/common/.mozilla/firefox" "snap"
    # Firefox ESR / Developer Edition (некоторые дистрибутивы)
    scan_firefox_profiles "$HOME_DIR/.mozilla/firefox-esr" "esr"
fi

if [ "$FF_FOUND" -eq 0 ]; then
    # Проверяем есть ли Firefox вообще в системе
    FF_EXISTS=0
    FF_SOURCE=""
    if cmd_exists firefox || cmd_exists firefox-esr; then
        FF_EXISTS=1; FF_SOURCE="native (через команду firefox)"
    fi
    if cmd_exists flatpak && flatpak list 2>/dev/null | grep -qi firefox; then
        FF_EXISTS=1; FF_SOURCE="${FF_SOURCE:+$FF_SOURCE + }flatpak (org.mozilla.firefox)"
    fi
    if cmd_exists snap && snap list 2>/dev/null | grep -qi firefox; then
        FF_EXISTS=1; FF_SOURCE="${FF_SOURCE:+$FF_SOURCE + }snap"
    fi
    if [ "$FF_EXISTS" -eq 1 ]; then
        if [ "$FF_PROFILE_FOUND" -eq 1 ]; then
            write_log "Firefox установлен ($FF_SOURCE). Профили найдены, но установленных пользовательских расширений нет."
        else
            write_log "Firefox установлен ($FF_SOURCE), но профили не найдены."
            write_log "  Возможные причины:"
            write_log "    1) Firefox ещё не запускался ни разу (профиль создаётся при первом запуске)."
            write_log "    2) Профиль в нестандартном месте. Проверьте: ls -la ~/.mozilla/ ~/.var/app/org.mozilla.firefox/ ~/snap/firefox/"
            write_log "    3) Запуск в sandbox / portable-режиме."
        fi
    else
...
if [ "$OS_TYPE" = "Darwin" ]; then
    ...
    if cmd_exists resolvectl; then
        ...
        # ПРИМЕЧАНИЕ: Команда `resolvectl statistics` НЕ ВЫЗЫВАЕТСЯ намеренно.
        # На SteamOS/KDE Plasma polkit требует GUI-аутентификацию
        # ("Чтобы создать дамп статистики, необходимо пройти аутентификацию").
        # Скрипт не должен повышать права — пропускаем сбор детальной статистики кэша.
    fi
fi
...
if [ "$OS_TYPE" = "Darwin" ]; then
    ...
    # Linux: определяем активный интерфейс.
    # ВАЖНО: `ip link show up` фильтрует по admin-флагу IFF_UP, а не по carrier.
    # USB-Ethernet без кабеля может быть admin-UP, но operstate=down (NO-CARRIER).
    # Поэтому:
    #   1) Сначала берём интерфейс дефолтного маршрута (гарантированно рабочий)
    #   2) Фоллбэк: перебираем интерфейсы, отфильтровав по operstate=up и carrier=1

    # Шаг 1: интерфейс дефолтного маршрута
    DEFAULT_IF=""
    if cmd_exists ip; then
        DEFAULT_IF="$(ip route show default 2>/dev/null | awk '/^default/ {for(i=1;i<=NF;i++) if($i=="dev") {print $(i+1); exit}}' | head -1)"
    fi
    if [ -z "$DEFAULT_IF" ] && cmd_exists route; then
        # Устаревший net-tools: route -n
        DEFAULT_IF="$(route -n 2>/dev/null | awk '/^0\.0\.0\.0/ {print $NF; exit}')"
    fi

    # Шаг 2: составляем список кандидатов. Первым — интерфейс дефолтного маршрута.
    # Затем — другие operstate=up интерфейсы (на случай если default вдруг виртуальный).
    CANDIDATES=""
    if [ -n "$DEFAULT_IF" ]; then
        CANDIDATES="$DEFAULT_IF"
    fi
    # Перебираем все не-lo интерфейсы, оставляем только operstate=up
    for i in $(ls /sys/class/net/ 2>/dev/null | grep -vE '^lo$|^docker|^veth|^br-|^virbr'); do
        OP="$(cat /sys/class/net/"$i"/operstate 2>/dev/null)"
        if [ "$OP" = "up" ] && [ "$i" != "$DEFAULT_IF" ]; then
    ...
    for iface in $CANDIDATES; do
        ...
        # Определяем, является ли интерфейс беспроводным (Wi-Fi)
        # На Linux беспроводные интерфейсы имеют /sys/class/net/<iface>/phy80211 или /wireless/
        IS_WIRELESS=0
        ...
        if [ "$IS_WIRELESS" -eq 1 ]; then
            # --- Wi-Fi интерфейс: sysfs/ethtool не работают, используем iw/iwconfig ---
            WIFI_BITRATE=""
            # Способ 1: iw dev <iface> link — современный инструмент
            if cmd_exists iw; then
                WIFI_BITRATE="$(iw dev "$iface" link 2>/dev/null | awk -F: '/tx bitrate:/ {print $2}' | sed 's/^[[:space:]]*//')"
            fi
            # Способ 2: iwconfig Bit Rate= (устаревший, но шире поддерживается)
            if [ -z "$WIFI_BITRATE" ] && cmd_exists iwconfig; then
                WIFI_BITRATE="$(iwconfig "$iface" 2>/dev/null | grep -oE 'Bit Rate[=:][^ ]+ [GM]b/s' | sed 's/Bit Rate[=:]//')"
            fi
            # Способ 3: nmcli
            if [ -z "$WIFI_BITRATE" ] && cmd_exists nmcli; then
                WIFI_BITRATE="$(nmcli -t -f IN-USE,RATE dev wifi list 2>/dev/null | grep -E '^\*' | head -1 | cut -d: -f2)"
            fi
            if [ -n "$WIFI_BITRATE" ]; then
                LINK_SPEED_STR="Wi-Fi: ${WIFI_BITRATE} / ${iface}"
            else
                MTU_VAL="$(cat /sys/class/net/"$iface"/mtu 2>/dev/null || echo '?')"
                LINK_SPEED_STR="Wi-Fi: скорость канала недоступна / ${iface} MTU:${MTU_VAL}"
            fi
            break  # Нашли Wi-Fi — выходим
        fi

        # --- Проводной интерфейс: sysfs → ethtool ---
        # Способ 1: sysfs /sys/class/net/<iface>/speed (в Mbps)
        SPEED_RAW="$(cat /sys/class/net/"$iface"/speed 2>/dev/null)"
        if [ -n "$SPEED_RAW" ] && [ "$SPEED_RAW" != "-1" ] 2>/dev/null; then
            SPEED="$SPEED_RAW"
            DUPLEX="$(cat /sys/class/net/"$iface"/duplex 2>/dev/null || echo 'N/A')"
        fi
        # Способ 2: ethtool (если sysfs не дал)
        if [ -z "$SPEED" ] && cmd_exists ethtool; then
            ETH_OUT="$(ethtool "$iface" 2>/dev/null)"
        ...
        if [ -n "$SPEED" ]; then
            ...
            # Проводной интерфейс без доступной скорости: VPS, виртуализация, USB-Ethernet без ethtool
            MTU_VAL="$(cat /sys/class/net/"$iface"/mtu 2>/dev/null || echo '?')"
            OPER_STATE="$(cat /sys/class/net/"$iface"/operstate 2>/dev/null || echo '?')"
            LINK_SPEED_STR="Недоступна (VPS/виртуализация или USB-адаптер) | ${iface} MTU:${MTU_VAL} State:${OPER_STATE}"
            break
        fi
    done
fi
