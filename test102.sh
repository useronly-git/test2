...
if [ "$OS_TYPE" = "Darwin" ]; then
    ...
    # sntp может вернуть ошибку (сервер недоступен и т.д.) — обрабатываем корректно
    if cmd_exists sntp; then
        SNTP_RESULT="$(sntp -S time.apple.com 2>/dev/null)"
        SNTP_EXIT=$?
        if [ "$SNTP_EXIT" -eq 0 ] && [ -n "$SNTP_RESULT" ]; then
            write_log "NTP (sntp): $SNTP_RESULT"
        else
            write_log "NTP: Не удалось получить ответ от time.apple.com (sntp вернул код $SNTP_EXIT)"
        fi
    else
...
# --- Функция определения проводного подключения на macOS ---
# Вызывается когда Wi-Fi не подключен — определяем Ethernet через system_profiler
_macos_detect_wired_connection() {
    # Определяем активный интерфейс через route
    local active_if
    active_if="$(route -n get default 2>/dev/null | awk '/interface:/ {print $2}')"

    if [ -n "$active_if" ]; then
        # Проверяем media тип активного интерфейса
        local media_info status_info inet_info
        ...
        # Пробуем получить детали через system_profiler SPNetworkDataType
        local sp_ethernet
        sp_ethernet="$(system_profiler SPNetworkDataType 2>/dev/null)"

        # Ищем активный сервис (тот, у которого есть IPv4)
        local hw_type="Проводное подключение"
        local link_speed=""

        # Определяем тип hardware по имени интерфейса
        if echo "$active_if" | grep -q '^en'; then
            # Ищем скорость в system_profiler SPEthernetDataType
            local sp_eth_detail
            sp_eth_detail="$(system_profiler SPEthernetDataType 2>/dev/null)"
            if [ -n "$sp_eth_detail" ]; then
                link_speed="$(echo "$sp_eth_detail" | grep -i 'Speed:' | head -1 | awk -F: '{print $2}' | sed 's/^[[:space:]]*//')"
            fi

            # Определяем тип: USB Ethernet, Thunderbolt Ethernet, встроенный
            if echo "$sp_eth_detail" | grep -qi 'USB'; then
                hw_type="USB Ethernet адаптер"
            elif echo "$sp_eth_detail" | grep -qi 'Thunderbolt'; then
                hw_type="Thunderbolt Ethernet"
            fi

            # Если system_profiler не дал скорость — парсим из media
        ...
        # Показываем ifconfig активного интерфейса
