...
do_ping() {
    ...
    if [ "$OS_TYPE" = "Darwin" ]; then
        # macOS: -W принимает миллисекунды, добавляем -v для подробного вывода
...
if [ "$OS_TYPE" = "Darwin" ]; then
    ...
    # Объединяем через запятую (tr заменяет переводы строк)
    VPN_IFS="$(ifconfig 2>/dev/null | grep -E '^(utun|ipsec|ppp)[0-9]' | awk -F: '{print $1}' | tr '\n' ',' | sed 's/,$//')"
    if [ -n "$VPN_IFS" ]; then
...
if [ "$OS_TYPE" = "Darwin" ]; then
    ...
    if [ -x "$AIRPORT" ]; then
        ...
        # airport недоступен (macOS 14+ Sonoma) — пробуем через networksetup
        if [ -n "$WIFI_SERVICE" ] && [ "$WIFI_POWER" = "On" ]; then
            ...
            write_log "Тип подключения: Проводное (Ethernet) или Wi-Fi выключен."
            ifconfig en0 2>/dev/null | grep -E 'inet |media:|status:' >> "$LOG_FILE"
        fi
    fi
else
...
if [ -d "$FF_PROFILES_DIR" ]; then
    for profile_dir in "$FF_PROFILES_DIR"/*/; do
        ...
        if [ -f "$ADDONS_FILE" ]; then
            ...
            # Парсим extensions.json: ищем блоки аддонов с помощью grep -oP (Perl regex)
            # Если grep -P недоступен (macOS) — фоллбэк на простой grep
            # Формат: "addons":[{..."name":"NAME"..."active":true/false..."type":"extension"...}]
            # Нам нужны только type=extension (не тема, не языковой пакет)
            #
            # Стратегия: извлекаем каждый объект аддона целиком, потом из каждого берём name и active
            # Без jq это сложно — используем построчный парсинг по разделителю },{

            # Безопасный способ: awk разбивает JSON на отдельные addon-блоки
            awk -v RS='},{' '
                /\"type\":\"extension\"/ {
                    # Извлекаем имя
                    name = ""
                    ...
                    # Если name пустой, пробуем defaultLocale.name
                    ...
                    # Извлекаем id если имя всё ещё пустое
                    if (name == "" && match($0, /"id":"[^"]+"/)) {
                    ...
                    # Пропускаем системные (mozilla.org, system addons)
            ...
            # Считаем кол-во найденных расширений
...
if [ "$FF_FOUND" -eq 0 ]; then
    if [ -d "$FF_PROFILES_DIR" ]; then
        write_log "Firefox установлен, но файл extensions.json не найден (нет расширений или профиль не использовался)."
    else
...
if cmd_exists mtr; then
    ...
    mtr -4 --report --report-cycles 30 --no-dns "$MTR_IP" 2>/dev/null >> "$LOG_FILE"

    # Извлекаем jitter первого хопа для итогов (парсим вывод mtr)
    # Запускаем ещё раз для парсинга (или парсим из лога — но проще отдельно)
    ...
    # -4 = принудительно IPv4, -n = без DNS, -m 25 = макс хопов, -w 2 = таймаут
    ...
    echo "$HOP_IPS" | while IFS=' ' read -r hop_num hop_ip; do
        [ -z "$hop_ip" ] && continue
        # Пингуем хоп 10 раз (IPv4, quiet mode для парсинга статистики)
        PING_OUTPUT="$(do_ping -c 10 -q "$hop_ip")"
    ...
    # --- Ни mtr ни traceroute не установлены ---
    # НЕ заменяем на ping — он не даёт информации о промежуточных хопах
    write_log "  ! ВНИМАНИЕ: Утилиты mtr и traceroute не установлены."
    ...
    write_log "  !   macOS:         brew install mtr"
...
# --- Определяем CGNAT ---
IS_CGNAT=0
# Проверяем только если получили валидный IPv4 адрес
if [ "$PUB_IP" != "Error" ] && echo "$PUB_IP" | grep -qE '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$'; then
    ...
    if echo "$PUB_IP" | grep -qE '^100\.(6[4-9]|[7-9][0-9]|1[01][0-9]|12[0-7])\.'; then
        ...
        # Но также проверяем что IP не является приватным (10.x, 172.16-31.x, 192.168.x)
        if [ -n "$LOCAL_IPS" ] && ! echo "$LOCAL_IPS" | grep -qF "$PUB_IP"; then
            # Публичный IP не совпадает с локальным — но это может быть обычный NAT роутера
            # CGNAT определяем только по RFC 6598 диапазону, остальное — просто NAT
            if echo "$PUB_IP" | grep -qE '^(10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.)'; then
                IS_CGNAT=1  # Публичный сервис вернул приватный IP — что-то странное
            fi
            # Обычный NAT роутера — не считаем CGNAT
        fi
    fi
elif [ "$PUB_IP" != "Error" ]; then
    # Получен IPv6 вместо IPv4 — значит IPv4 недоступен или за NAT
    write_log "Публичный IPv4 не получен, получен IPv6: $PUB_IP"
fi
